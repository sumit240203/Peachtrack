...
if ($role === '102') {
    ...
    $sqlRecent = "SELECT t.Tip_Amount, {$saleColExpr} AS Sale_Amount, t.Is_It_Cash, s.Start_Time, t.Tip_Time
                  FROM tip t
                  JOIN shift s ON s.Shift_ID = t.Shift_ID
    ...
    if (!$stmt) {
        ...
        $sqlRecent = "SELECT t.Tip_Amount, {$saleColExpr} AS Sale_Amount, t.Is_It_Cash, s.Start_Time
                      FROM tip t
                      JOIN shift s ON s.Shift_ID = t.Shift_ID
                      WHERE s.Employee_ID = ?" . ($hasIsDeleted ? " AND (t.Is_Deleted IS NULL OR t.Is_Deleted = 0)" : "") . "
                      ORDER BY s.Start_Time DESC, t.Tip_ID DESC
